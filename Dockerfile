...
WORKDIR /opt/shinobi
RUN echo '-u root -pnight -e "source /opt/shinobi/sql/user.sql"' | mysql
RUN echo '-u root -pnight -e "source /opt/shinobi/sql/framework.sql"' | mysql
RUN echo '-u root --database ccio -e "source /opt/shinobi/sql/default_data.sql"' | mysql
RUN npm install
