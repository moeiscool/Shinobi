...
WORKDIR /opt/shinobi
CMD mysql -u root -pnight -e "source /opt/shinobi/sql/user.sql" || true
CMD mysql -u root -pnight -e "source /opt/shinobi/sql/framework.sql" || true
CMD mysql -u root -pnight --database ccio -e "source /opt/shinobi/sql/default_data.sql" || true
RUN npm install
